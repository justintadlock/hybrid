...
/** Begin. **/
		if ( 'post' !== $wp_query->post->post_type && 'page' !== $wp_query->post->post_type && 'attachment' !== $wp_query->post->post_type ) {

			$post_type = get_post_type_object( $wp_query->post->post_type );
			$parent_page = get_page_by_path( $post_type->rewrite['slug'] );
			...
			//if ( $parent_page )
			//	$trail[]  = '<a href="' . get_permalink( $parent_page->ID ) . '" title="' . get_the_title( $parent_page->ID ) . '">' . get_the_title( $parent_page->ID ) . '</a>';
/** end. */
		...
		elseif ( is_single() && 'post' == $wp_query->post->post_type ) {
...
/* Tax parent page.*/
			...
			$parent_page = get_page_by_path( $taxonomy->rewrite['slug'] );

			$parent_id = $parent_page->ID;

			/* Try a second level if there's a '/' in the rewrite slug. */
			if ( $parent_id == 0 ) {
				preg_match_all( "/\/.*?\z/", $taxonomy->rewrite['slug'], $matches );

				if ( isset( $matches ) ) {
					$matches = array_reverse( $matches );
					foreach ( $matches as $match ) {
						$path = str_replace( $match[0], '', $taxonomy->rewrite['slug'] );
